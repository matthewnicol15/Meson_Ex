...
void Tree_Reader_New_Skim3_MesonEx(){

  gROOT->ProcessLine(".L ./Loader.C+");
  TFile *f = new TFile("../Trees/Skim3_Pass1_Tree_040620_02.root");
  TTree *t1 = (TTree*)f->Get("Skim3_Tree_040620_02");
  ...
  TFile fileOutput1("../New_Data/Skim3_MesonEx_Pass1_Analysis_without_p_190820_01.root","recreate");
  ...
  //PID only cut
  auto* hmiss_mass_all=new TH1F("miss_all","MM^2(e' K^{+} K^{-} p);MM^2(e' K^{+} K^{-} p) [GeV];Counts",100,-1,1);
  auto* hmissp=new TH1F("hmissp","MM(e' K^{+} K^{-});MM(e' K^{+} K^{-}) [GeV];Counts",200,-1,3);
  ...
  auto* hinv_K_P_K_M=new TH1F("hinv_K_P_K_M","M(K^{+} K^{-});M(K^{+} K^{-}) [GeV];Counts",200,0.8,3);
  ...
  auto* hmiss_mass_allc=new TH1F("miss_allc","MM^2(e' K^{+} p #pi^{-}) after cuts on delta beta and momentum of K^{+} and p;MM^2(e' K^{+} p #pi^{-}) [GeV];Counts",100,-1,1);
  ...
  auto* hinv_K_P_K_Mc=new TH1F("hinv_K_P_K_Mc","M(K^{+} K^{-});M(K^{+} K^{-}) [GeV];Counts",200,0.8,3);
  ...
  auto* hinv_K_P_K_Mt=new TH1F("hinv_K_P_K_Mt","M(K^{+} K^{-});M(K^{+} K^{-}) [GeV];Counts",200,0.8,3);
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(v_kp.size()==1 && v_km.size()==1 && v_el.size()==1 /*&& v_pr.size()==1*/){
      ...
      missp = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - el_corrected - v_kp.at(0) - v_km.at(0);
      inv_K_P_K_M = v_kp.at(0) + v_km.at(0);
      photon = (TLorentzVector)*readbeam - el_corrected;
      // Testing vertex time and path functions
      hpath_kp->Fill(v_path_kp.at(0));
      hvertex_time_kp->Fill(v_vertex_time_kp.at(0));
      hphoton->Fill(photon.M());
      ...
      if(fabs(v_delta_beta_kp.at(0))<0.01 && v_P_kp.at(0)>0.2 && v_P_kp.at(0)<2.6 /*&& fabs(v_delta_beta_pr.at(0))<0.01 && v_P_pr.at(0)>0.2 && v_P_pr.at(0)<4.8*/){
        ...
        // if(fabs(missall.M2())<0.05 && missall.Rho()<0.5){
        if(missp.M()>0.8 && missp.M()<1.1){
